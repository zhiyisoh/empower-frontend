...
export default {
    ...
    mounted() {
            console.log('https://3.222.113.2:8080/api/logging/co2sum/' + this.$store.state.auth.user.id);
            axios.get('https://3.222.113.2:8080/api/logging/co2sum/' + this.$store.state.auth.user.id, {
                headers: {
            ...
            axios.get('https://3.222.113.2:8080/api/logging/co2sum', {
                headers: {
                    'Authorization': 'Bearer ' + this.$store.state.auth.user.accessToken
                }
            }).then(response => {
